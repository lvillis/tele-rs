...
#[tokio::test]
async fn default_user_agent_does_not_expose_library_name() -> Result<(), DynError> {
    let response =
        r#"{"ok":true,"result":{"id":42,"is_bot":true,"first_name":"bot","username":"bot"}}"#;
...
#[tokio::test]
async fn user_agent_override_replaces_generic_default() -> Result<(), DynError> {
    let response =
        r#"{"ok":true,"result":{"id":42,"is_bot":true,"first_name":"bot","username":"bot"}}"#;
    let (base_url, handle) = spawn_server_with_checks(
        "/bot123:abc/getMe",
